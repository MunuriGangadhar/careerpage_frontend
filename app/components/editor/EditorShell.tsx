...
export default function EditorShell() {
  ...
  const updateSection = (id: string, field: string, val: string | boolean) =>
    setCompany((p) => p
      ? { ...p, contentSections: p.contentSections.map((s) => s.id === id ? { ...s, [field]: val } : s) }
      : null);

  const removeSection = (id: string) =>
    setCompany((p) => p
      ? { ...p, contentSections: p.contentSections.filter((s) => s.id !== id) }
      : null);
  ...
  const addSection = () => {
    ...
    setCompany((p) => p
      ? { ...p, contentSections: [...p.contentSections, { id, type: 'custom', title: 'New Section', content: '', visible: true, order }] }
      : null);
  };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="max-w-2xl mx-auto">
                ...
                <Card title="Typography & Shape">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        value={company.brandTheme?.fontFamily || 'DM Sans'}
                    ...
                    <div>
                      ...
                      <select
                        value={company.brandTheme?.borderRadius || 'md'}
            ...
            {tab === 'jobs' && (
              <>
                {/* Header row with + Post Job button */}
                <div className="flex items-center justify-between mb-5">
                  <h2 className="text-lg font-bold text-white" style={{ fontFamily: 'var(--font-display)' }}>
                    Job Listings
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      href={`/${slug}/jobs/new`}
                      className="flex items-center gap-1.5 text-xs px-4 py-2 rounded-xl font-bold transition-all hover:opacity-85"
                      style={{ background: 'var(--gold)', color: '#0A0A0F', fontFamily: 'var(--font-display)' }}
                    >
                      + Post Job
                    </Link>
                ...
                <div className="rounded-xl px-4 py-3 mb-4 flex items-center gap-3"
                  ...
                  <p className="text-xs" style={{ color: 'rgba(255,255,255,0.5)' }}>
                    Click{' '}
                    <strong style={{ color: 'var(--gold)' }}>+ Post Job</strong>{' '}
                    to add a new position. Click{' '}
                    <strong style={{ color: 'rgba(255,255,255,0.7)' }}>Edit</strong>{' '}
                    on any job to update details or hide it from candidates.
                  </p>
                </div>
                ...
                {jobs.length === 0 ? (
                  <div className="text-center py-16 rounded-2xl"
                    style={{ border: '1px dashed rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.25)' }}>
                    <p className="text-4xl mb-3">◎</p>
                    <p className="text-sm font-medium mb-4" style={{ fontFamily: 'var(--font-display)' }}>
                      No jobs posted yet
                    </p>
                    <Link
                      href={`/${slug}/jobs/new`}
                      className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-xl text-xs font-bold transition-all hover:opacity-85"
                      style={{ background: 'var(--gold)', color: '#0A0A0F', fontFamily: 'var(--font-display)' }}
                    >
                      + Post your first job
                    </Link>
                    ...
                    {jobs.map((job) => (
                      <div
                        ...
                        style={{
                          ...
                          border: `1px solid ${job.isActive ? 'rgba(255,255,255,0.07)' : 'rgba(255,255,255,0.03)'}`,
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs mt-0.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                            {job.location} · {job.workPolicy} · {job.employmentType} · {job.experienceLevel}
                          </p>
                          {job.salaryRange && (
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          ...
                          {/* Posted date */}
                          <span className="text-xs hidden md:block" style={{ color: 'rgba(255,255,255,0.2)' }}>
                            {daysAgo(job.postedAt)}
                          </span>

                          {/* View on careers page */}
                          <a
                            href={`/${slug}/careers/${job.slug}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="hover:text-white/60 transition-colors px-1.5 py-1 rounded-lg"
